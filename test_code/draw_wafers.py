...
def draw_wafer(renderer, cx, cy, radius, label=""):
    ...
    - 흰색 배경 + 짙은 회색 테두리만
    """
    renderer.wafer_type2(cx, cy, radius,
                         fill=Colors.WHITE,
                         stroke=Color(80, 80, 80),
                         stroke_width=2,
                         notch_width=radius * 0.15,
                         bg_color=Colors.WHITE)

    # 라벨
    if label:
        renderer.text(label, cx, cy + radius + 25,
                      size=16, color=Color(80, 80, 80), align="center")
...
def main():
    ...
    renderer.text(
        ...
        canvas_w // 2, 35, size=24, color=Color(80, 80, 80), align="center", bold=True,
    )
